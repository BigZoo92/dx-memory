...
import { useDashboardSummary } from '@signalops/flow-api-client'
      ...
      <div className="so-grid-detail">
          ...
          {summary.mostCriticalSignals.map((signal) => (
            ...
            >
              ...
              <span
                ...
                style={{ color: riskBand(signal.riskScore) === 'critical' ? '#d92d20' : undefined }}
              >
                {signal.riskScore}
              </span>
            </Link>
        ...
        <div className="so-grid-sidecol">
          ...
          <AiCard title="AI recommendation">
            3 critical signals from Partner API share the same latency pattern. Grouping them into
            one incident may reduce qualification time by ~22%.
          </AiCard>
        </div>
      </div>
          ...
          <table className={styles.incidentTable}>
            ...
            <tbody>
              {summary.recentIncidents.map((incident) => (
                <tr key={incident.id}>
                  <td>
                    <Link to="/incidents" className={styles.incidentLink}>
...
export function DashboardScreen() {
  ...
  return (
    <div className="so-page">
          ...
          <>
            ...
            <Button
              ...
              onClick={() =>
                summary &&
                downloadTextFile(
                  'signalops-overview.json',
                  dashboardToJson(summary),
      ...
      />
      <QueryState query={query}>{(data) => <DashboardBody summary={data} />}</QueryState>
    </div>
  )
}
