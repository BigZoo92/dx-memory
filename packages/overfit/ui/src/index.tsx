// Overfit UI primitives. Same visual language as the other variants; the only visible difference
// anywhere in the product is the variant badge value.
...
/** Risk-trend badge (the AI-task capability). Always renders a text label, never color alone. */
export function RiskTrendBadge({ trend }: { trend: RiskTrend }) {
  return (
    <Badge hue={riskTrendHue(trend)}>
      <span aria-hidden>{riskTrendArrow(trend)}</span> {riskTrendLabel(trend)}
    </Badge>
  )
}
