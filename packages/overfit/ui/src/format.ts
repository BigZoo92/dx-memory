// Presentation helpers shared by the UI primitives. Pure functions; no React.
...
// ---- risk trend (the AI-task capability) ----
export function riskTrendLabel(t: RiskTrend): string {
  return t === 'up' ? 'Rising' : t === 'down' ? 'Falling' : 'Stable'
}

export function riskTrendHue(t: RiskTrend): string {
  // Rising risk is bad (red), falling is good (green), stable is neutral (grey).
  return t === 'up' ? 'red' : t === 'down' ? 'green' : 'grey'
}

export function riskTrendArrow(t: RiskTrend): string {
  return t === 'up' ? '▲' : t === 'down' ? '▼' : '▬'
}
