...
Overfit ships six local quality gates. They are lightweight and fast by design: each is a small, deterministic check you can run before pushing. The weight of Overfit is not in the gates themselves - it is in how many coordinated things must be true for one product change to pass all of them.
...
6. Artifact manifest - `pnpm overfit:artifacts:check`. Verifies release-critical Docker, lockfile, OpenAPI, endpoint-manifest and CI inputs match the committed provenance manifest.
